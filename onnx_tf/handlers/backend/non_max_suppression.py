...
class NonMaxSuppression(BackendHandler):
  ...
  def _common(cls, node, **kwargs):
    ...
    if center_point_box == 1:
      ...
      y1 = tf.subtract(y_centers, tf.math.xdivy(heights, 2))
      x1 = tf.subtract(x_centers, tf.math.xdivy(widths, 2))
      y2 = tf.add(y_centers, tf.math.xdivy(heights, 2))
      x2 = tf.add(x_centers, tf.math.xdivy(widths, 2))
